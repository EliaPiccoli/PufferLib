...
from models.wsa import WSA_Robot
...
# class ChannelLastWrapper(gym.Wrapper):
#     """
#     A wrapper to ensure that image observations have channels last (HWC) format.
#     """
#     def __init__(self, env):
#         super().__init__(env)
#         self.observation_space = self._convert_observation_space(env.observation_space)

#     def _convert_observation_space(self, space):
#         if isinstance(space, gym.spaces.Box) and len(space.shape) == 3:  # Image space
#             return gym.spaces.Box(
#                 low=space.low.transpose(1, 2, 0),
#                 high=space.high.transpose(1, 2, 0),
#                 shape=(space.shape[1], space.shape[2], space.shape[0]),
#                 dtype=space.dtype,
#             )
#         elif isinstance(space, gym.spaces.Dict):  # Dict space
#             return gym.spaces.Dict({k: self._convert_observation_space(v) for k, v in space.spaces.items()})
#         return space

#     def observation(self, obs):
#         if isinstance(obs, dict):
#             return {k: self.observation(v) for k, v in obs.items()}
#         if isinstance(obs, np.ndarray) and len(obs.shape) == 3:  # Image observation
#             return obs.transpose(1, 2, 0)  # Convert CHW to HWC
#         return obs
...
if __name__ == "__main__":
    ...
    agent = WSA_Robot(envs, device=device, emb_size=256).to(device)
