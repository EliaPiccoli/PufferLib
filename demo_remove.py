...
def train(args, make_env, policy_cls, rnn_cls, wandb,
    ...
    cur_n_models = policy._modules['policy']._modules['policy'].n_models
    while cur_n_models > 0:
        first = True
        current_steps = data.global_step
        while data.global_step < current_steps + additional_timesteps:
            if first:
                print("****** Eval Before Remove ******")
...
if __name__ == '__main__':
    ...
    if args['mode'] == 'train':
        wandb = None
        if args['track']:
            wandb = init_wandb(args, env_name + "-remove", id=args['exp_id'])
