...
def train(args, make_env, policy_cls, rnn_cls, wandb,
    ...
    import hackatari_test as ht
    e_name = policy._modules['policy']._modules['policy'].env_name
    device = policy._modules['policy']._modules['policy'].device
    if e_name == "breakout":
        ht.breakout_hacktari_eval(policy, device)
    elif e_name == "pong":
        ht.pong_hacktari_eval(policy, device)
...
if __name__ == '__main__':
    ...
    if args['mode'] == 'train':
        wandb = None
        if args['track']:
            wandb = init_wandb(args, env_name + "-hack", id=args['exp_id'])
