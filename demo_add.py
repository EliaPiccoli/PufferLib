...
def sweep(args, env_name, make_env, policy_cls, rnn_cls):
    ...
    def main():
        try:
            wandb = init_wandb(args, name=env_name, id=args['exp_id'])
...
def train(args, make_env, policy_cls, rnn_cls, wandb,
    ...
    cur_n_models = 1
    policy._modules['policy']._modules['policy'].n_models = cur_n_models
    steps_interval = train_config.total_timesteps
    
    while data.global_step < train_config.total_timesteps*4:
        if data.global_step > steps_interval*cur_n_models:
                print("****** Eval Before Add ******")
                print("Current_n_models:", policy._modules['policy']._modules['policy'].n_models)
                policy._modules['policy']._modules['policy'].print_ww = True
                clean_pufferl.evaluate(data)
                clean_pufferl.mean_and_log(data)
                policy._modules['policy']._modules['policy'].print_ww = False
                print("*********************************")
...
if __name__ == '__main__':
    ...
    if args['mode'] == 'train':
        wandb = None
        if args['track']:
            wandb = init_wandb(args, env_name + "-add", id=args['exp_id'])
